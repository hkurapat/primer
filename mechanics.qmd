...
<!-- Not sure where this chapter goes. It makes use of models with two independent variables, which means four parameters. -->
...
*When comparing two people, one about 1 standard deviation worth of years older than the other, we expect the older person to earn about `r scales::comma(round(fixef(fit_1_s)["s_age", "Estimate"], -3))` dollars more.*
...
See @roas for more useful discussion.
...
### Variables in the `trains` data set
...
- The 95% confidence interval for  `att_end` is equal to the coefficient-- `r round(fixef(fit_1_model)["Intercept", "Estimate"], 2)`-- plus or minus two standard errors. This shows the estimate for the `att_end` where the commuters were under treatment and **were not** in the control group. 
...
The causal effect of the variable `treatmentControl` is `r round(fixef(fit_1_model)["treatmentControl", "Estimate"], 2)`. This means that, compared with the predicted `att_end` for groups under treatment, those in the control group have a predicted attitude that is almost one entire point lower. As we can see, this is a large and well estimated coefficient. Recall that this means, roughly, that the 95% confidence interval excludes zero. To calculate the 95% confidence interval, we take the coefficient plus or minus two standard errors. As we can see, the 95% confidence interval *does* exclude zero, suggesting that `treatment` is a worthy variable. 
...
The variable `att_start`, with a coefficient of `r round(fixef(fit_1_model)["att_start", "Estimate"], 2)`, is also meaningful. 
...
When comparing `liberalTRUE` with our `(Intercept)`, recall that the `Intercept` is calculating income for the case where `liberal == FALSE`. As we can see, then, the coefficient for `liberalTRUE`, `r scales::comma(round(fixef(fit_2_model)["liberalTRUE", "Estimate"], 1))`, shows that the  liberals in our data set make less on average than non-liberals. The coefficient is large relative to the `Intercept` and, with rough mental math, we see that the 95% confidence interval excludes 0. Therefore, `liberal` is a helpful variable.
