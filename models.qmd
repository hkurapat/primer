...
> *What are the odds that, if we meet two voters, both will have voted for Biden?*
...
> *Using data from a YouGov poll of 1,559 US adult citizens, conducted March 10 - 12, 2024, we seek to understand what proportion of voters will support Biden in the 2024 election. In particular, we want to estimate the odds that, if we meet two voters, both will have voted for Biden?*
...
Let's use `fit_bern` to answer this question with the help of `add_predicted_draws()`. Instead of having just one row in `newdata`, we now have two rows, one for each person for whom we need a posterior.
