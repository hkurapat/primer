...
*Covariates*: Besides exposure to Spanish-speakers, it is not clear what other covariates might be useful in answering the question. We need to explore our dataset to see what we have available. (Hint: We have a variable about political orientation, `liberal`,  which will prove useful.)
...
*The population is not the set of units for which we have data.* That is the dataset. The population is the larger --- potentially much larger --- set of units about whom we want to make inferences. *The parameters in our models refer to the population, not to the dataset.*
